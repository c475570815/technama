...
//导入excel
    ...
    public function property(){
        ...
        $_id=$_POST['id'];
        $teacher=TeacherModel::get( $_id);

        $property_data=[];
        $property_data["total"]=count($teacher);

        $properties=array();
        foreach ($teacher as $field_name=>$field_value){
            $property["name"]=array_key_exists($field_name,TeacherModel::$fieldNameMap)?TeacherModel::$fieldNameMap[$field_name]:$field_name;
            $property["value"]=$field_value;
//            $property["group"]="";
//            $property["editor"]="text";
            $properties[]=$property;
        }
        $property_data["rows"]=$properties;
